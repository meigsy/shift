...
# Data source to read the surface_preferences SQL file
data "local_file" "surface_preferences_sql" {
  filename = "${path.module}/sql/surface_preferences.sql"
}
...
resource "google_bigquery_table" "surface_preferences" {
  ...
  view {
    query = templatefile("${path.module}/sql/surface_preferences.sql", { project_id = var.project_id })
    use_legacy_sql = false
  }
...
# Data source to read the state_estimator input view SQL file
data "local_file" "v_state_estimator_input_v1_sql" {
  filename = "${path.module}/sql/v_state_estimator_input_v1.sql"
}

# Data source to read the state_estimator unprocessed view SQL file
data "local_file" "v_state_estimator_unprocessed_v1_sql" {
  filename = "${path.module}/sql/v_state_estimator_unprocessed_v1.sql"
}

# BigQuery view for state estimator input
resource "google_bigquery_table" "v_state_estimator_input_v1" {
  dataset_id = google_bigquery_dataset.shift_data.dataset_id
  table_id   = "v_state_estimator_input_v1"
  project    = var.project_id

  view {
    query = templatefile("${path.module}/sql/v_state_estimator_input_v1.sql", { project_id = var.project_id })
    use_legacy_sql = false
  }
...
# BigQuery view for state estimator unprocessed
resource "google_bigquery_table" "v_state_estimator_unprocessed_v1" {
  dataset_id = google_bigquery_dataset.shift_data.dataset_id
  table_id   = "v_state_estimator_unprocessed_v1"
  project    = var.project_id

  view {
    query = templatefile("${path.module}/sql/v_state_estimator_unprocessed_v1.sql", { project_id = var.project_id })
    use_legacy_sql = false
  }
  ...
  depends_on = [
    ...
    google_bigquery_table.watch_events,
    google_bigquery_table.state_estimates,
    google_bigquery_table.v_state_estimator_input_v1
  ]
}
